...
export class ProductosSucursalesComponent implements OnInit {
  ...
  constructor(private _productosSucursalesService: ProductosSucursalesService,
    ...
    this.productoSucursalModelGetId = new ProductosSucursales('','',0,0,'','');

        this.token = this._productosSucursalesService.obtenerToken();

  }

  ngOnInit(): void {this.getProductosSucursales1();
  ...
  getProductosSucursalesId(idProducto){
    this._productosSucursalesService.obtenerProductosSucursalesId(idProducto).subscribe(
      (response) => {
  ...
  putProductosSucursales(){
    this._productosSucursalesService.simularVenta(this.productoSucursalModelGetId).subscribe(
      (response)=>{
